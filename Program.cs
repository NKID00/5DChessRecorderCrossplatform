    ...
    class Program
    {
        ...
        private static string GenerateFEN(ChessBoard board, string timeline, string turn)
        {
            ...
            for (int y = board.height - 1; y >= 0; y--)
            {
                ...
                {
                    var p = board.Pieces[x * board.height + y];
                    if (p.IsEmpty)
                    {
                        var i = 0;
                        while (x < board.width && board.Pieces[x * board.height + y].IsEmpty)
                        {
                            i += 1;
                            x += 1;
                        }
                    ...
                    {
                        pieces += GenerateFENSymbol(p);
                    }
                }
            }
            return $"\r\n[{pieces}:{timeline}:{turn}:{(board.cbm.isBlacksMove == 0 ? 'w' : 'b')}]";
        }
        ...
        private static void RecordGames(DataInterface di)
        {
            ...
            var startTime = "";
            var localPlayer = -1;
            var memLocEvenTimelines = new FiveDChessDataInterface.MemoryHelpers.MemoryLocation<int>(di.GetGameHandle(), di.MemLocChessArrayPointer.Location, -0x34); // 0 if odd number of starting timelines, -1 if even
            ...
            while (true)
            {
                ...
                if (lastTime != 0 && (thisTime - 1) * 3 <= (lastTime - 1) * 2)
                {//Should always tick if thisTime==1 or thisTime==0
                    try
                    {
                        // player.Play();
                    }
                ...
                if (cnt != oldCnt || turn != oldTurn || state != oldState)
                {
                    ...
                    var evenTLs = memLocEvenTimelines.GetValue();
                    ...
                    if (oldCnt == 0)
                    {
                        ...
                        localPlayer = memLocWhichPlayerIsLocal.GetValue(); //0 white, 1 black, 2 both
                    }
                    fen = "\r\n[Board \"custom\"]";
                    lastRun = $"[Mode \"5D\"]\r\n[Result \"{GameStateToResult(state)}\"]\r\n[Date \"{startDate}\"]\r\n[Time \"{startTime}\"]\r\n[Size \"{di.GetChessBoardSize().Width}x{di.GetChessBoardSize().Height}\"]";
                    if (localPlayer == 1)
                    {
                        lastRun += "\r\n[White \"Opponent\"]";
                        //lastRun+="\r\n[Black \"tesseract\"]";
                    ...
                    for (int i = 0; i < cbs.Count; i++)
                    {
                        ...
                        {
                            // Add to FEN
                            fen += GenerateFEN(board1, mkL(mem1.timeline), mkT(mem1.turn));
                            continue;
                        }
                        ...
                        if (mem.moveSourceL <= 0 && mem.moveSourceT <= 0 && mem.moveSourceX <= 0 && mem.moveSourceY <= 0
                                ...
                                && (mem1.creatingMoveNumber == mem.moveTurn || mem.moveTurn == -1))
                        {
                            fen += GenerateFEN(board1, mkL(mem1.timeline), mkT(mem1.turn));
                            continue;
                        }
